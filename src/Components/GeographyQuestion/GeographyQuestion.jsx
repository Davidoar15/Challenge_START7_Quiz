...
import { getGeography } from "../../Redux/actions";
import style from '../Question.module.css'
import { NavLink } from "react-router-dom";

function QuestionGeography({ geography, getGeography }) {
  ...
  useEffect(() => {
    if (currentQuestionIndex < geography.length) {
      const currentQuestion = geography[currentQuestionIndex];
      const options = [
  ...
  }, [currentQuestionIndex, geography]);
  ...
  const handleNextQuestion = () => {
    if (currentQuestionIndex < geography.length - 1) {
      ...
      {showResult ? (
        ...
        <div className={style.questionContainer}>
            <div className={style.containerQ}>
                ...
                <h2>{geography[currentQuestionIndex].question}</h2>
            </div>
...
export default connect(mapStateToProps, { getGeography })(QuestionGeography);